...
IssuerStakingController.sol lets issuers stake native MOCA tokens (via `msg.value`), with a required delay to unstake. Security is built in with limits, pausing, freezing, and emergency exit features. Only certain roles can take admin actions. 

The contract uses wrapped MOCA (wMOCA) as a fallback mechanism: if native MOCA transfers fail within a gas limit, tokens are automatically wrapped and transferred as wMOCA.
...
- Native MOCA token staking via `msg.value` with immediate effect
- No token approvals required (native MOCA)
...
- Functions: Pause, Unpause, Freeze, EmergencyExit
- Emergency exit function processes individual issuers
- Accessible when contract is frozen by EmergencyExitHandler or by issuers for their own funds
- Automatic wMOCA wrapping if native transfers fail
...
Issuers can stake native MOCA tokens directly to the contract via `msg.value`, with immediate effect on their staked balance and the global totals.
...
- **Staking requirements**: Any amount > 0 and ≤ `MAX_SINGLE_STAKE_AMOUNT` can be staked
...
uint256 public MAX_SINGLE_STAKE_AMOUNT;      // Maximum single stake amount
uint256 public MOCA_TRANSFER_GAS_LIMIT;     // Gas limit for native MOCA transfers
address public immutable WMOCA;             // Wrapped MOCA contract address
```
...
The `MAX_SINGLE_STAKE_AMOUNT` parameter prevents excessive single transactions:
...
## Wrapped MOCA Fallback Mechanism

The contract integrates with wrapped MOCA (wMOCA) as a safety mechanism:

**Purpose:**
- Some recipient addresses (e.g., smart contracts with complex fallback functions) may require more gas than standard EOA transfers
- If a native MOCA transfer fails within the configured gas limit, tokens are automatically wrapped to wMOCA and transferred instead

**Implementation:**
- `MOCA_TRANSFER_GAS_LIMIT` configures the gas limit for native transfers (minimum 2300, typical EOA requirement)
- If transfer fails, contract wraps native MOCA to wMOCA and transfers wMOCA to recipient
- Used in `claimUnstake()` and `emergencyExit()` functions
...
The primary staking function enables issuers to stake native MOCA tokens:
...
**Process Flow:**
1. **Validation**: Checks `msg.value` > 0 and ≤ `MAX_SINGLE_STAKE_AMOUNT`
2. **State Updates**: Increments issuer balance and global totals
3. **Native Transfer**: Native MOCA is sent with the transaction (`msg.value`)
...
5. **Token Transfer**: Transfers native MOCA to issuer (wraps to wMOCA if transfer fails within gas limit)
...
**Transfer Safety:**
- Attempts native MOCA transfer with `MOCA_TRANSFER_GAS_LIMIT` gas
- If transfer fails, automatically wraps to wMOCA and transfers wMOCA instead
- Ensures tokens are always recoverable regardless of recipient address type
...
**`setMaxSingleStakeAmount()`**: Updates maximum single stake limit
...
**`setMocaTransferGasLimit()`**: Updates gas limit for native MOCA transfers
...
- Only callable by IssuerStakingController admin
- Must be ≥ 2300 (minimum for EOA transfers)
- Used when attempting native MOCA transfers in `claimUnstake()` and `emergencyExit()`
- If native transfer fails within this limit, tokens are wrapped to wMOCA
- Emits event with old and new values
...
1. **Issuer Decision**: Issuer decides to stake native MOCA tokens
2. **Stake Transaction**: Issuer calls `stakeMoca()` with native MOCA sent as `msg.value`
3. **Contract Processing**:
   - Validates `msg.value` > 0 and ≤ `MAX_SINGLE_STAKE_AMOUNT`
   - Updates issuer balance and global totals
   - Native MOCA is held in contract balance
   - Emits staking event
4. **Confirmation**: Staking complete, tokens locked in contract

**Note**: No token approval step required since native MOCA is used.
...
6. **Contract Processing**:
   ...
   - Attempts native MOCA transfer to issuer (within gas limit)
   - If transfer fails, wraps to wMOCA and transfers wMOCA instead
...
- Callable by Emergency Exit Handler role OR by issuers for their own funds
- Must provide non-empty array of issuer addresses

**Access Control:**
- **Emergency Exit Handler**: Can process any issuer addresses in the array
- **Issuers**: Can only process their own address (array must contain only their address)
- If caller is neither Emergency Exit Handler nor the issuer address, transaction reverts

**Process:**
1. **Validation**: Confirms frozen state and non-empty address array
2. **Batch Processing**: Iterates through each issuer address
3. **Access Check**: Verifies caller has permission for each address
4. **Amount Calculation**: Sums staked and pending MOCA per issuer
5. **Token Transfer**: Attempts native MOCA transfer to each issuer (wraps to wMOCA if transfer fails within gas limit)
6. **State Cleanup**: Resets issuer balances and updates global totals
7. **Event Emission**: Records emergency exit with processed addresses and total MOCA

**Key Features:**
- **Targeted Recovery**: Processes specific issuers instead of transferring everything to treasury
...
- **Transfer Safety**: Automatically wraps to wMOCA if native transfers fail

**Recovery Implications:**
- **User Impact**: Affected issuers receive their MOCA (or wMOCA) directly
...
2. Can manipulate limits: Admin sets extreme parameters (very long delays, very low limits, very high gas limits)
3. Users affected by parameter changes
4. Remove malicious admin through AccessController
5. Worst case: Emergency exit recovers funds for specific issuers
...
**Note**: The contract uses native MOCA (not ERC20 MOCA) and includes automatic wMOCA wrapping as a fallback mechanism for transfers that fail within the configured gas limit.

```bash
╭---------------------------------------------------------------+-------------------+------------------+----------------+-----------------╮
| File                                                          | % Lines           | % Statements     | % Branches     | % Funcs         |
+=========================================================================================================================================+
| src/IssuerStakingController.sol                               | 100.00% (101/101) | 100.00% (98/98)  | 94.44% (51/54) | 100.00% (12/12) |
|---------------------------------------------------------------+-------------------+------------------+----------------+-----------------|
| src/LowLevelWMoca.sol                                         | 100.00% (6/6)     | 100.00% (5/5)    | 100.00% (1/1)  | 100.00% (1/1)   |
|---------------------------------------------------------------+-------------------+------------------+----------------+-----------------|
| src/libraries/EpochMath.sol                                   | 0.00% (0/18)      | 0.00% (0/19)     | 100.00% (0/0)  | 0.00% (0/9)     |
|---------------------------------------------------------------+-------------------+------------------+----------------+-----------------|
| test/1. IssuerStakingController/IssuerStakingController.t.sol | 100.00% (53/53)   | 100.00% (42/42)  | 100.00% (0/0)  | 100.00% (12/12) |
|---------------------------------------------------------------+-------------------+------------------+----------------+-----------------|
| test/utils/MockUSD8.sol                                       | 33.33% (2/6)      | 33.33% (1/3)     | 100.00% (0/0)  | 33.33% (1/3)    |
|---------------------------------------------------------------+-------------------+------------------+----------------+-----------------|
| test/utils/MockWMoca.sol                                      | 40.74% (11/27)    | 47.83% (11/23)   | 14.29% (1/7)   | 28.57% (2/7)    |
|---------------------------------------------------------------+-------------------+------------------+----------------+-----------------|
| test/utils/TestingHarness.sol                                 | 88.89% (16/18)    | 88.24% (15/17)   | 100.00% (0/0)  | 66.67% (2/3)    |
|---------------------------------------------------------------+-------------------+------------------+----------------+-----------------|
| Total                                                         | 82.53% (189/229)  | 83.09% (172/207) | 85.48% (53/62) | 63.83% (30/47)  |
╰---------------------------------------------------------------+-------------------+------------------+----------------+-----------------╯
```
