...
Unlike major protocols like Aerodrome that rely on external off-chain relayers for delegation management, VotingController processes all delegations entirely on-chain. This approach ensures:
...
### 2. **Historical Fee Tracking Problem/Solution**
...
- This leads to inaccuracies because fees could have been updated any time - a user claiming for some arbitrary past epoch would be subject to the prevailing `currentFee`.
...
To ensure fees are accurately applied for their respective epochs, we've implemented a historical fee tracking system:
...
- **Epoch-Specific Tracking**: When delegates vote, their current fee is logged to `delegateHistoricalFees` for that epoch
...
## 1. claimSubsidies: ratio downcast is safe

```solidity
// poolAllocatedSubsidies is uint128 (from storage)
uint128 poolAllocatedSubsidies = epochPools[epoch][poolId].totalSubsidiesAllocated;

// Both values from PaymentsController are uint256, in 1e6 precision
...
// Final calculation with downcast
uint128 subsidyReceivable = uint128((ratio * poolAllocatedSubsidies) / 1E18); 
```

### **1. Ratio is Bounded to [0, 1E18]**

`ratio = (verifierAccruedSubsidies × 1E18) / poolAccruedSubsidies`

By definition from PaymentsController:

- verifierAccruedSubsidies = subsidies accrued by ONE verifier for this pool
- poolAccruedSubsidies = TOTAL subsidies accrued by ALL verifiers for this pool

Since a single verifier's contribution cannot exceed the total pool contribution:
`verifierAccruedSubsidies ≤ poolAccruedSubsidies`

**Therefore:**

```bash
ratio = (verifierAccruedSubsidies × 1E18) / poolAccruedSubsidies
      ≤ (poolAccruedSubsidies × 1E18) / poolAccruedSubsidies
      = 1E18
```
**Conclusion: ratio ∈ [0, 1E18]**

### **2. Final Result is Bounded by poolAllocatedSubsidies**

`subsidyReceivable = (ratio × poolAllocatedSubsidies) / 1E18`

Since ratio ≤ 1E18:

```bash
subsidyReceivable = (ratio × poolAllocatedSubsidies) / 1E18
                  ≤ (1E18 × poolAllocatedSubsidies) / 1E18
                  = poolAllocatedSubsidies
```

**Conclusion: subsidyReceivable ≤ poolAllocatedSubsidies**

### **3. `poolAllocatedSubsidies` is stored as uint128**

- since `subsidyReceivable ≤ poolAllocatedSubsidies ≤ type(uint128).max`

**Conclusion: The downcast from uint256 to uint128 CANNOT overflow because the result is mathematically bounded by a value that is already uint128.**

-The ratio represents "what fraction of the pool's total accrued subsidies belongs to this verifier" — this fraction is always between 0% and 100% (0 to 1E18 in fixed-point).

- When you multiply a uint128 value (poolAllocatedSubsidies) by a fraction ≤ 1 and divide back, the result cannot exceed the original uint128 value.

## 2. Potential Rounding Dust Accumulation in Reward/Subsidy Distribution

In `processEpochRewardsSubsidies()`:

`uint128 poolSubsidies = (poolVotes * epochTotalSubsidiesAllocated) / epochTotalVotes;`

In `claimPersonalRewards()`:

`uint128 userRewards = (userVotes * poolRewards) / poolTotalVotes;`

Integer division causes rounding down. 
- The sum of all `poolSubsidies` will be slightly less than `epochTotalSubsidiesAllocated`. 
- Similarly, sum of all `userRewards` will be less than `poolRewards`.

**Example:**
- epochTotalSubsidiesAllocated = 100
- 3 pools with equal votes (33.33% each)
- Each pool gets 33, total distributed = 99, dust = 1

**Current Mitigation:** `withdrawUnclaimedSubsidies` and `withdrawUnclaimedRewards` sweep residual amounts to treasury.

## Epoch State Flags  

The epoch lifecycle flags are:
1. isSubsidiesSet - set in depositEpochSubsidies()
2. isFullyProcessed - set in processEpochRewardsSubsidies()
3. isEpochFinalized - set in finalizeEpoch()
4. isRewardsWithdrawn - set in withdrawUnclaimedRewards()
5. isSubsidiesWithdrawn - set in withdrawUnclaimedSubsidies()

When TOTAL_ACTIVE_POOLS == 0 in depositEpochSubsidies(); skips processEpochRewardsSubsidies() and finalizeEpoch(). 
...
BUT totalActivePools = 0 and poolsProcessed = 0

## Why uint128 Is Safe for Vote & Reward Accounting

Voting Power & uint128 Head-Room 

- Total MOCA supply: 8.89 billion (8.89 × 10⁹ tokens, 18 decimals → 8.9 × 10²⁷ units).
- VotingEscrowMoca returns voting power ≤ staked principal; therefore any single balance, balanceAtEpochEnd() result, or epoch-level totalVotes is bounded by 8.9 × 10²⁷—roughly 11 orders of magnitude below uint128 max (3.4 × 10³⁸).
- Epoch-scoped fields (Epoch.totalVotes, PoolEpoch.totalVotes) reset each epoch, eliminating long-term accumulation risk.
- Long-lived accumulators (Pool.totalVotes) grow by at most full-supply votes per epoch.
-- 50 years × 52 epochs ≈ 2 600 epochs → worst-case total ≈ 2.3 × 10³¹, still 10⁷× under uint128 cap.
- Reward / subsidy math uses 256-bit intermediates, then down-casts only after division; resulting values remain < 8.9 × 10²⁷, safely within uint128.
- Safety margin: ≥ 10 million-fold against overflow under current supply & epoch cadence.

Conclusion: Choosing uint128 for vote and reward accounting is sound; overflow is not realistically reachable without a >10⁵× supply increase or centuries of uninterrupted maximum-supply voting.  No realistic overflow risk exists under current tokenomics.

Maximum possible voting power
 – Full supply locked → 8.89 B tokens × 1e18 = 8.9 × 10²⁷ units.

## Kill Switch & Operational Dependencies

**Sequential cron pipeline** 
- Each epoch relies on three privileged calls (`depositEpochSubsidies`, `processEpochRewardsSubsidies`, `finalizeEpoch`) in that exact order. 
- Missing or reverting any step blocks the epoch indefinitely; the only escape hatch is destructive (`forceFinalizeEpoch`).

**forceFinalizeEpoch impact** 
- A DEFAULT_ADMIN_ROLE call to `forceFinalizeEpoch` marks rewards and subsidies as “withdrawn” without transferring tokens.
- Users can no longer claim, yet balances remain in the contract until manually swept elsewhere.

**Freeze + emergency exit** 

- Once `freeze` is invoked, only the emergency-exit handler can call `emergencyExit`, which unconditionally transfers *all* esMOCA plus native MOCA (registration fees) to the treasury. 

**Operational risk** 
- Compromising or misconfiguring any of these roles (cron admin, monitor, default admin, emergency handler) can halt rewards, confiscate user balances, or drain the contract.
- None of the functions are timelocked or multi-sig-gated by default.

**Recommended operator practice**

- Run the cron pipeline via monitored automation with replay protection and signed task configs.
- Require timelock + multi-sig approval before invoking `forceFinalizeEpoch`, `freeze`, or `emergencyExit`.
- Keep high-frequency calls (cron, monitor) on separately permissioned keys with revocation procedures.
- Schedule periodic `VotingEscrowMoca.updateAccountsAndPendingDeltas` / `updateDelegatePairsAndPendingDeltas` jobs so that claims cannot be bricked by stale checkpoints.
