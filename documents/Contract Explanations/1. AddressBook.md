...
#### Global Admin Updates via Ownership Transfer
...
The AddressBook contract overrides the internal `_transferOwnership(address newOwner)` function to ensure that global admin changes are synchronized across the protocol. This is the **only way** to update the global admin address.
...
| `transferOwnership(address)`     | Owner         | Initiate ownership & global admin transfer |
| `acceptOwnership()`              | Pending Owner | Complete ownership & global admin transfer |
