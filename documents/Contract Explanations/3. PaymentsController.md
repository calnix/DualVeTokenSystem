...
- [2. Design Choices](#2-design-choices)
  ...
  - [Subsidy System: Fixed Tier Array](#subsidy-system-fixed-tier-array)
...
- Verifiers stake MOCA to qualify for subsidy tiers
- Uses a fixed array of 10 subsidy tiers, ordered ascending by `mocaStaked`
- Finds the highest tier the verifier qualifies for (closest largest match)
...
9. `deductBalance` currently does not check that the user’s credentials are actually verified (success or attempted). This means that the user can make a request to the api to get a deduction signature and deduct the verifier’s balance to the schema’s issuer without providing any proof of verification.

10. There is no check for whether a poolId exists when adding or whitelisting it. Currently, as the voting controller is not completed, we cannot add a check by calling the voting controller to determine if a pool exists or is active.

11. `_usedSignatureHashes[signerAddress][signatureHash]` is logged on-chain (without any revert guard) for optional off-chain replay/front-run monitoring by the SDK; execution flow is unchanged because caller role checks on `deductBalance` and `deductBalanceZeroFee` remain the only on-chain gate.

12. Caller gating for deduct flows: if the caller is not whitelisted with `WHITELISTED_DEDUCT_CALLER_ROLE`, they must be the `userAddress` parameter; this applies to both `deductBalance` and `deductBalanceZeroFee`.

**The above risks have been made known to product and deemed to accepted.**
...
mapping(uint128 epoch => mapping(uint128 poolId => uint256 totalSubsidies)) _epochPoolSubsidies;
mapping(uint128 epoch => mapping(uint128 poolId => mapping(address verifierAdminAddress => uint256))) _epochPoolVerifierSubsidies;

// Fee tracking for rewards distribution
mapping(uint128 epoch => mapping(uint128 poolId => DataTypes.FeesAccrued)) _epochPoolFeesAccrued;
mapping(uint128 epoch => DataTypes.FeesAccrued) _epochFeesAccrued;
```
...
```solidity
// Primary actor mappings - keyed by admin address (msg.sender at creation)
mapping(address issuerAdminAddress => DataTypes.Issuer) internal _issuers;
mapping(address verifierAdminAddress => DataTypes.Verifier) internal _verifiers;

// Schema mapping - keyed by deterministic bytes32 ID
mapping(bytes32 schemaId => DataTypes.Schema) internal _schemas;## Constructor & Initialization
```
...
1. Create Profile: Call `createIssuer(assetManagerAddress)` - uses `msg.sender` as admin address
2. Create Schemas: Call `createSchema(fee)` for each credential type
3. Manage Fees: Update schema fees as needed (decreases immediate, increases delayed)
4. Claim Revenue: Periodically claim accumulated fees to asset manager address

#### Issuer Structure

`function createIssuer(address assetManagerAddress) external`
- The caller's address (`msg.sender`) becomes the issuer's admin address (mapping key)
- No explicit issuerId is returned; the admin address serves as the identifier
...
- **Admin Address** (mapping key): Set to `msg.sender` at creation; serves as the issuer's control address for contract interactions (e.g., schema management, fee updates, address changes). [*Immutable - cannot be changed*]
- `assetManagerAddress`: Provided at creation; acts as the designated wallet for claiming accrued fees.

The `assetManagerAddress` is dedicated solely to fee withdrawals, while the admin address (caller) is required for all issuer-initiated contract actions. This separation enables issuers to maintain distinct operational and treasury controls, supporting flexible security postures.

The admin address is immutable, and cannot be changed by an issuer. 
- If their admin address is compromised, they will have to create a new issuer profile with a new admin address.
- Fees unclaimed under their previous issuer profile would be lost, assuming the attacker immediately called `updateAssetManagerAddress` and `claimFees`, the moment the admin address was exploited.

**Rationale for address-based identification:**
- Simplicity: Direct mapping lookup without ID indirection
- Gas efficiency: No ID generation overhead for issuer creation
- Enables updates of asset manager addresses without recreating profiles
...
1. Create Profile: Call `createVerifier(signerAddress, assetManagerAddress)` - uses `msg.sender` as admin address
...
`function createVerifier(address signerAddress, address assetManagerAddress) external`
- The caller's address (`msg.sender`) becomes the verifier's admin address (mapping key)
- No explicit verifierId is returned; the admin address serves as the identifier
...
```
- **Admin Address** (mapping key): Assigned at verifier creation (`msg.sender`); controls verifier configuration, key rotation, and operational actions. [*Immutable - cannot be changed*]
...
- Verifiers are identified by their admin address (`msg.sender` at creation)
- The admin address is used as the mapping key: `_verifiers[adminAddress]`
- No separate `verifierId` is generated; the admin address IS the identifier

Verifiers are assigned distinct `admin` (mapping key), `signer`, and `assetManager` addresses, enabling independent management of operational permissions, signature authority, and treasury controls. This modular approach supports secure key rotation and flexible access strategies.

The admin address is immutable, and cannot be changed by a verifier. 
- If their admin address is compromised, they will have to create a new profile with a new admin address.
...
**Rationale for Address-Based Identification:**
- Simplicity: Direct mapping lookup without ID indirection
- Gas efficiency: No ID generation overhead for verifier creation
...
`function createSchema(uint128 fee) external returns (bytes32)`

When an issuer (identified by `msg.sender`) calls `createSchema`, the contract:
1. Generates a unique `bytes32` schemaId using deterministic generation with collision prevention
2. Links it to the issuer's admin address
...
```solidity
    struct Schema {
        address issuer;            // issuer's admin address
...
uint256 salt = _issuerSchemaNonce[msg.sender];

bytes32 schemaId = keccak256(abi.encode("SCHEMA", msg.sender, totalSchemas, salt));

// if non-zero issuer; this schemaId has been taken
...
**Key Points:**
- Checks `_schemas` mapping for collisions (issuer address != address(0))
- Increments salt until unique ID found
- Ensures global uniqueness for all schemas
...
updatePoolId(bytes32 schemaId, uint128 poolId)  // Admin function**Pool Association Rules:**
- Default: `poolId = 0` (no voting pool; poolIds start at 1)
...
## Subsidy System: Fixed Tier Array

Verifiers stake MOCA to qualify for subsidy percentages. The system uses a fixed array of 10 tiers:
...
**Tier Matching Logic:**
- Tiers must be ordered ascending by `mocaStaked`
- The system finds the **highest tier** the verifier qualifies for (closest largest)
- Loops backwards through tiers to find the first qualifying tier

**Design Benefits:**
- Predictability: Clear tiers without ambiguity
- Simplicity: Fixed array with backward iteration for efficient lookup
- Flexibility: Admin can set/update tiers via `setVerifierSubsidyTiers()`
- Contiguity: Tiers are set as a contiguous block from index 0

**Admin Functions:**
- `setVerifierSubsidyTiers(mocaStaked[], subsidyPercentages[])`: Overwrites all tiers
- `clearVerifierSubsidyTiers()`: Deletes all tiers
...
**Caller gate**
- Only the user (`msg.sender == userAddress`) or addresses with `WHITELISTED_DEDUCT_CALLER_ROLE` can call `deductBalance` / `deductBalanceZeroFee`. This is the sole on-chain caller check; signature hashes are only logged for optional off-chain monitoring.
...
- Checks if schema is associated with a whitelisted pool
...
**`whitelistPool(uint128 poolId, bool isWhitelisted)`:** Whitelist or un-whitelist a pool
...
4. PaymentsController:
    ...
    - Books subsidies if applicable (pool whitelisted + verifier staked)
...
Epoch N Timeline:
├─ Day 1-3: Schema has no pool (`poolId = 0` or not whitelisted)
...
├─ Day 4: Admin sets poolId = 0 or un-whitelists pool
│ → Association removed
...
### Scenario 2: Contract Upgrade Failure  
...
### Scenario 3: Native MOCA Transfer Failures
...
### Entity Retrieval
- `getIssuer(address issuer)` - Returns complete Issuer struct
- `getVerifier(address verifier)` - Returns complete Verifier struct  
...
- `getIssuerSchemaNonce(address issuer)` - Returns creation nonce for schema generation
...
- `getEpochPoolSubsidies(uint128 epoch, uint128 poolId)` - Returns total subsidies for pool in epoch
- `getEpochPoolVerifierSubsidies(uint128 epoch, uint128 poolId, address verifier)` - Returns verifier's subsidies for pool in epoch
- `getVerifierAndPoolAccruedSubsidies(uint128 epoch, uint128 poolId, address verifier, address caller)` 
   [*Potentially called by VotingController for subsidy claims; validates caller is verifier's asset manager*]

### Fee Tracking
- `getEpochPoolFeesAccrued(uint128 epoch, uint128 poolId)` - Returns protocol and voter fees for pool in epoch
- `getEpochFeesAccrued(uint128 epoch)` - Returns total protocol and voter fees for epoch

### Pool Management
- `checkIfPoolIsWhitelisted(uint128 poolId)` - Returns true if pool is whitelisted
