...
```
- `verifierId`: Deterministically generated `bytes32` identifier assigned at verifier creation: `bytes32 verifierId = keccak256(abi.encode("VERIFIER", msg.sender, salt));`
...
PaymentsController takes the following dependencies at deployment (stored as immutable or configurable):
- **AccessController**: Passed at deployment, stored as immutable - used for all role checks
...
- **VERIFIER_CONTRACT**: Passed at deployment, can be updated by admin - must match caller of `deductBalance()` and `deductBalanceZeroFee()`
- **EIP-712 name/version**: Passed at deployment - for signature validation
...
- **Direct Dependencies**: Contract dependencies passed at deployment and stored as immutable (except VERIFIER_CONTRACT which is updatable)
- **Clean Interfaces**: Standardized role checking via AccessController interface
...
- **No Central Registry**: Eliminates dependency on AddressBook, simplifying upgrade path
