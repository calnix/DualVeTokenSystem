...
1. **Selection Phase** (`selectRedemptionOption`)
   - User chooses a redemption option, `amount`, and `minExpectedReceivable`
...
```
This prevents gaming the system with micro-transactions that would avoid penalties due to rounding, and protects users from unexpected fee changes between transaction submission and execution.
...
**`selectRedemptionOption(uint256 redemptionOption, DataTypes.RedemptionOption calldata expectedOption, uint256 redemptionAmount, uint256 minExpectedReceivable)`**
...
- `minExpectedReceivable` protects against receiving less than expected due to rounding
...
9. **Slippage Protection**: `minExpectedReceivable` parameter protects users from receiving less than expected due to rounding
