...
1. Choose amount (MOCA and/or esMOCA) and duration (min 3 epochs from current, max 2 years)
...
- Requires at least 3 epochs remaining from current epoch
...
- **Emergency Exit Handler**: Can return funds when frozen
- **Voting Controller Admin**: Manages delegate registration settings
- **Voting Escrow Moca Admin**: Can set gas limits for MOCA transfers
- **Cron Job**: Can create locks for multiple users via `createLockFor`

## Three-Stage Emergency System
1. **Pause**: Stops all operations (reversible)
2. **Freeze**: Permanent shutdown (one-way)
3. **Emergency Exit**: Returns all locked tokens to users without state updates

## Built-in Protections
- Minimum lock amounts prevent dust attacks (1e13 wei minimum)
- 3-epoch minimum for operations ensures effectiveness
...
2. **Batch Processing**: Epoch alignment enables efficient loops; batch update functions available
...
6. **View Functions**: Read-only balance queries avoid state updates
...
**Batch Operations**:
- `createLockFor`: CRON_JOB_ROLE can create locks for multiple users
- `updateAccountsAndPendingDeltas`: Batch update stale accounts
- `updateDelegatePairs`: Batch update user-delegate pairs for reward claims
...
- 3-epoch minimum lock duration requirements
- Direct delegate switching functionality
- Emergency exit without state updates
