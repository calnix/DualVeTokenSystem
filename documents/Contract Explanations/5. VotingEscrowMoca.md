...
- Delegate individual locks (not partial amounts)
   - Change delegates using `delegationAction(..., Switch)` or reclaim power via `delegationAction(..., Undelegate)` anytime
...
5. Delegation state tracked via `lock.delegate` field
   ...
   **Three Lock Delegation States**
   ...
   A lock can exist in one of three delegation states, determining who holds voting power (VP) and how modifications are handled:

   | State | Condition | Who Holds VP | Behavior on increaseAmount/Duration |
   |-------|-----------|--------------|---------------------------------------------|
   | **Undelegated** | `lock.delegate == address(0)` | Owner | Owner gets immediate VP increase |
   | **Pending Delegation** | `lock.delegationEpoch > currentEpoch` | Owner (until next epoch) | Owner gets immediate VP, pending deltas queued for delegate |
   | **Active Delegation** | `lock.delegate != 0` AND `lock.delegationEpoch <= currentEpoch` | Delegate | Delegate gets immediate VP, NO pending deltas |
...
- increaseAmount
- increaseDuration
   - `delegationAction` (Delegate/Switch/Undelegate)
...
- **Undelegating then redelegating in same epoch**: Changes queued to next epoch, mathematically cancel out
   - **Switching delegates directly**: Single atomic operation via `delegationAction(..., Switch)`
...
**`delegateRegistrationStatus(address delegate, bool toRegister)`**
- **Caller**: Only VotingController contract
- **Purpose**: Register (`toRegister=true`) or unregister (`toRegister=false`) a delegate
- **Storage**: Sets `isRegisteredDelegate[delegate] = toRegister`
- **Event**: Emits `DelegateRegistrationStatusUpdated(delegate, toRegister)`
- **Note**: Registration fee collected by VotingController, not veMoca2. Unregistering does not affect existing delegations; users retain delegated locks.
...
**DoS Prevention via Delegate Switching**:
- An attack vector exists where a user with large voting power repeatedly switches delegates to overflow variables or consume gas.
- Analysis shows this is impractical:
   - Requires ~1.18 quintillion switches to overflow bias variables.
   - With 30M block gas limit and ~150k gas per switch, max switches per epoch is ~242 million.
   - It would take ~1.33 billion switches to attempt an overflow even if holding the entire token supply.
   - Thus, the attack is economically and technically infeasible.
...
**Purpose**: CRON_JOB_ROLE helper to batch update stale accounts to current epoch
...
**Purpose**: CRON_JOB_ROLE helper to batch update stale user-delegate pairs
...
**Undelegate + Delegate = Switch**:
- If a user undelegates and then delegates to a new person in the same epoch, it is mathematically identical to a direct switch.
- The contract schedules two opposing updates for the next epoch:
   - Undelegate: +VP to user, -VP from old delegate
   - Delegate: -VP from user, +VP to new delegate
   - Net Result: User change is 0, Old Delegate loses VP, New Delegate gains VP.
...
- Undelegate + Delegate in same epoch = Switch (mathematically identical)
   - Direct switching via `delegationAction(..., Switch)` function atomically handles:
