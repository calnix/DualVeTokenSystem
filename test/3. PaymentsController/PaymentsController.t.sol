...
contract StateT0_DeployAndCreateSubsidyTiers_Test is StateT0_Deploy {
    ...
    function test_Deploy() public {
        ...
        assertEq(address(paymentsController.accessController()), address(accessController), "accessController not set correctly");
    ...
    function testRevert_Deploy_AccessControllerIsZeroAddress() public {
        vm.expectRevert(Errors.InvalidAddress.selector);
        new PaymentsController(address(0), protocolFeePercentage, voterFeePercentage, feeIncreaseDelayPeriod, address(mockWMoca), address(mockUSD8), MOCA_TRANSFER_GAS_LIMIT, address(verifierContract), "PaymentsController", "1");
    }

    function testRevert_Deploy_WMOCAIsZeroAddress() public {
        vm.expectRevert(Errors.InvalidAddress.selector);
        new PaymentsController(address(accessController), protocolFeePercentage, voterFeePercentage, feeIncreaseDelayPeriod, address(0), address(mockUSD8), MOCA_TRANSFER_GAS_LIMIT, address(verifierContract), "PaymentsController", "1");
    }

    function testRevert_Deploy_USD8IsZeroAddress() public {
        vm.expectRevert(Errors.InvalidAddress.selector);
        new PaymentsController(address(accessController), protocolFeePercentage, voterFeePercentage, feeIncreaseDelayPeriod, address(mockWMoca), address(0), MOCA_TRANSFER_GAS_LIMIT, address(verifierContract), "PaymentsController", "1");
    }

    function testRevert_Deploy_MOCA_TRANSFER_GAS_LIMITIsLessThan2300() public {
        vm.expectRevert(Errors.InvalidGasLimit.selector);
        new PaymentsController(address(accessController), protocolFeePercentage, voterFeePercentage, feeIncreaseDelayPeriod, address(mockWMoca), address(mockUSD8), 2300 - 1, address(verifierContract), "PaymentsController", "1");
    }
    ...
    function testRevert_Deploy_ProtocolFeePercentageIsGreaterThan100Pct() public {
        ...
        new PaymentsController(address(accessController), Constants.PRECISION_BASE + 1, voterFeePercentage, feeIncreaseDelayPeriod, address(mockWMoca), address(mockUSD8), MOCA_TRANSFER_GAS_LIMIT, address(verifierContract), "PaymentsController", "1");
    }

    function testRevert_Deploy_VoterFeePercentageIsGreaterThan100Pct() public {
        vm.expectRevert(Errors.InvalidPercentage.selector);
        new PaymentsController(address(accessController), protocolFeePercentage, Constants.PRECISION_BASE + 1, feeIncreaseDelayPeriod, address(mockWMoca), address(mockUSD8), MOCA_TRANSFER_GAS_LIMIT, address(verifierContract), "PaymentsController", "1");
    }

    function testRevert_Deploy_FeeIncreaseDelayPeriodIsZero() public {
        vm.expectRevert(Errors.InvalidDelayPeriod.selector);
        new PaymentsController(address(accessController), protocolFeePercentage, voterFeePercentage, 0, address(mockWMoca), address(mockUSD8), MOCA_TRANSFER_GAS_LIMIT, address(verifierContract), "PaymentsController", "1");
    }

    function testRevert_Deploy_FeeIncreaseDelayPeriodIsLessThanEpochDuration() public {
        vm.expectRevert(Errors.InvalidDelayPeriod.selector);
        new PaymentsController(address(accessController), protocolFeePercentage, voterFeePercentage, EpochMath.EPOCH_DURATION - 1, address(mockWMoca), address(mockUSD8), MOCA_TRANSFER_GAS_LIMIT, address(verifierContract), "PaymentsController", "1");
    }
...
contract StateT1_CreateIssuerVerifiers_Test is StateT1_CreateIssuerVerifiers {
    ...
    function testCannot_CreateSchema_InvalidIdCollision() public {
        ...
        bytes32 expectedSchemaId = generateUnusedSchemaId(issuer1_Id, totalSchemas);
    ...
    function testCan_CreateSchema() public {
        ...
        bytes32 expectedSchemaId1 = generateUnusedSchemaId(issuer1_Id); 
...
contract StateT3_Verifier1DepositUSD8_Test is StateT3_Verifier1DepositUSD8 {
    ...
    //------- state transition: deductBalance() -----------
        function testCannot_DeductBalance_InvalidCaller() public {
            vm.expectRevert(Errors.InvalidCaller.selector);
            vm.prank(address(0xdeadbeef)); // not the verifier contract
        }
        ...
        function testCan_DeductBalance_AppliesPendingFeeIncrease() public {
            ...
            vm.prank(issuer1);
            paymentsController.updateSchemaFee(issuer1_Id, schemaId1, amount);
...
contract StateT8_IssuerIncreasedFeeIsAppliedAfterDelay_Test is StateT8_IssuerIncreasedFeeIsAppliedAfterDelay {
    ...
    function testCan_Issuer3CreatesSchemaWith0Fees() public {
        ...
        bytes32 expectedSchemaId3 = generateUnusedSchemaId(issuer3_Id);
...
contract StateT10_Verifier1StakeMOCA_Test is StateT10_Verifier1StakeMOCA {
    ...
    //------------------------------ tests for unstakeMoca ------------------------------
        ...
        function testCan_Verifier1_UnstakeMOCA_ReceiveNative() public {
            ...
            // Unstake
            vm.startPrank(verifierContract);
                paymentsController.unstakeMoca(verifier1_Id, amount);
            vm.stopPrank();
...
contract StateT12_IssuerClaimsAllFees_Test is StateT12_IssuerClaimsAllFees {
    ...
    function testCan_IssuerUpdateAssetManagerAddress() public{
        ...
        vm.prank(issuer1);
        address newAssetManagerAddress = paymentsController.updateAssetManagerAddress(issuer1_Id, issuer1_newAssetAddress);
...
abstract contract StateT13_IssuerChangesAssetManagerAddress is StateT12_IssuerClaimsAllFees {
    ...
    function setUp() public virtual override {
        ...
        vm.prank(issuer1);
        paymentsController.updateAssetManagerAddress(issuer1_Id, issuer1_newAssetAddress);
...
contract StateT13_IssuerChangesAssetManagerAddress_Test is StateT13_IssuerChangesAssetManagerAddress {
    ...
    //------------------------------ common negative tests for updateAssetAddress ------------------------------
        ...
        function testCannot_UpdateAssetAddress_WhenAssetAddressIsZeroAddress() public {
            vm.expectRevert(Errors.InvalidAddress.selector);
            vm.prank(issuer1);
            paymentsController.updateAssetManagerAddress(issuer1_Id, address(0));
        }

        function testCannot_UpdateAssetAddress_WhenCallerIsNotIssuerAdmin() public {
            vm.expectRevert(Errors.InvalidCaller.selector);
            vm.prank(issuer2);
            paymentsController.updateAssetManagerAddress(issuer1_Id, address(1));
        }

        function testCannot_UpdateAssetAddress_WhenInvalidIdIsGiven() public {
            vm.expectRevert(Errors.InvalidId.selector);
            vm.prank(issuer1);
            paymentsController.updateAssetManagerAddress(bytes32(0), address(1));
        }
    ...
    //------------------------------ state transition: verifier updateAssetAddress ------------------------------
        function testCannot_UpdateAssetAddress_WhenCallerIsNotVerifierAdminAddress() public {
            vm.expectRevert(Errors.InvalidCaller.selector);
            vm.prank(verifier2);
            paymentsController.updateAssetManagerAddress(verifier1_Id, address(1));
        }
        ...
        function testCan_Verifier1UpdateAssetManagerAddress() public {
            ...
            vm.prank(verifier1);
            paymentsController.updateAssetManagerAddress(verifier1_Id, verifier1_newAssetManagerAddress);
...
abstract contract StateT14_VerifierChangesAssetAddress is StateT13_IssuerChangesAssetManagerAddress {
    ...
    function setUp() public virtual override {
        ...
        vm.prank(verifier1);
        paymentsController.updateAssetManagerAddress(verifier1_Id, verifier1_newAssetAddress);
    }
}
...
contract StateT19_DeductBalanceCalledForSchema1AfterFeeIncreaseAndNewDelay_Test is StateT19_DeductBalanceCalledForSchema1AfterFeeIncreaseAndNewDelay {
    ...
    function testCannot_PaymentsControllerAdmin_WithdrawProtocolFees_NoTreasuryAddressSet() public {
        // modify storage to set accessController.TREASURY() to address(0)
        stdstore
            .target(address(accessController))
            .sig("TREASURY()")  
            .checked_write(address(0));
        assertTrue(accessController.TREASURY() == address(0), "Treasury address should be 0");
...
contract StateT21_PaymentsControllerAdminFreezesContract_Test is StateT21_PaymentsControllerAdminFreezesContract {
    ...
    // ------ Contract paused: no fns can be called except: unpause or freeze -------
        ...
        // ------ Common functions for issuer and verifier ------
            function test_updateAssetManagerAddress_revertsWhenPaused() public {
                vm.expectRevert(Pausable.EnforcedPause.selector);
                vm.prank(issuer1);
                paymentsController.updateAssetManagerAddress(issuer1_Id, makeAddr("newAssetManagerAddress"));
            }
    ...
    // ------ Functions that should NOT revert when paused -------
        ...
        function test_viewFunctions_workWhenPaused() public view {
            ...
            paymentsController.accessController();
...
contract StateT22_PaymentsControllerAdminFreezesContract_Test is StateT22_PaymentsControllerAdminFreezesContract {
    ...
    // ------ emergencyExitVerifiers ------
        ...
        function testCan_EmergencyExitHandlerCall_EmergencyExitVerifiers_InvalidVerifierId() public {
            ...
            uint256 beforeContractBalance = address(paymentsController).balance;
            ...
            uint256 afterContractBalance = address(paymentsController).balance;
            ...
            assertEq(afterContractBalance, beforeContractBalance, "No tokens should be transferred for invalid verifierId");
        }
        ...
        function testCan_EmergencyExitHandler_EmergencyExitVerifiers() public {
            ...
            uint256 beforeVerifier1MOCABalance = verifier1Asset.balance;
            uint256 beforeVerifier2MOCABalance = verifier2Asset.balance;
            uint256 beforeVerifier3MOCABalance = verifier3Asset.balance;
            ...
            uint256 afterVerifier1MOCABalance = verifier1Asset.balance;
            uint256 afterVerifier2MOCABalance = verifier2Asset.balance;
            uint256 afterVerifier3MOCABalance = verifier3Asset.balance;
    ...
    // ------ emergencyExitFees ------
        ...
        function testCan_EmergencyExitHandler_EmergencyExitFees() public {
            ...
            address treasuryAddress = accessController.TREASURY();
